...
const PostInteractions = ({
  ...
  count: { comments: number; likes: number; rePosts: number };
  isLiked?: boolean;
  isReposted?: boolean;
  isSaved?: boolean;
  postId: number;
  ...
  }, [postId, addOptimisticCount, optimisticCount.isLiked, user, username]);

  const rePostAction = useCallback(() => {
    if (!user) return;

    if (!optimisticCount.rePosts) {
      socket.emit("sendNotification", {
        receiverUsername: username,
        data: {
          senderUsername: user.username,
    ...
    startTransition(async () => {
      ...
      await rePost(postId); // Assuming likePost handles repost logic
      setState((prev) => {
        return {
          ...prev,
          rePosts: prev.isReposted ? prev.rePosts - 1 : prev.rePosts + 1,
  ...
  }, [postId, addOptimisticCount, optimisticCount.rePosts, user, username]);

  const saveAction = useCallback(() => {
    startTransition(async () => {
      addOptimisticCount("save");
      await savePost(postId); // Assuming likePost handles save logic
      setState((prev) => {
        return {
          ...prev,
          isSaved: !prev.isSaved,
